...
class BoneZoom: UIView {
    ...
    convenience init(imageView: UIImageView) {
        ...
        self.navView.titleLabel.text = "預覽圖片"
    ...
    fileprivate lazy var navView: BoneZoomNav = {
        ...
        navView.closeBtn.addTarget(self, action: #selector(self.hideAnimate), for: UIControl.Event.touchUpInside)
        navView.isShow = true
        return navView
    }()
    ...
    func showAnimate() {
        UIView.animate(
            ...
            animations: {
                ...
                self.newImage.frame.size.height = UIScreen.main.bounds.size.height
...
extension BoneZoom: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let top = scrollView.contentOffset.y * -1 - 44
        print(top)
